...
final class AppConfig {
    ...
    private static final String KEY_DIGITAL_BELL_ENABLED = "digitalBellEnabled";
    ...
    static boolean digitalBellEnabled(Context context) {
        return preferences(context).getBoolean(KEY_DIGITAL_BELL_ENABLED, true);
    }

    static void save(Context context, String backendUrl, String agentToken, boolean digitalBellEnabled) {
        preferences(context)
                .edit()
                .putString(KEY_BACKEND_URL, backendUrl == null ? "" : backendUrl.trim())
                .putString(KEY_AGENT_TOKEN, agentToken == null ? "" : agentToken.trim())
                .putBoolean(KEY_DIGITAL_BELL_ENABLED, digitalBellEnabled)
                .apply();
    }
